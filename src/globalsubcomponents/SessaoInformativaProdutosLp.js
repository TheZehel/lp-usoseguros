        ...
        <div className="mx-auto grid max-w-2xl grid-cols-1 gap-x-8 gap-y-16 sm:gap-y-20 lg:mx-0 lg:max-w-none lg:grid-cols-2">
          <div className="lg:pr-8 lg:pt-4">
            <div className="lg:max-w-lg">
              <h2 className="text-base font-semibold leading-7 text-grayUso">
                {produto.title}
              </h2>
              ...
              <button
                ...
                className="bg-grayUso hover:bg-bluePrime2 text-white font-bold py-2 px-4 rounded w-2/4"
              >
                {produto.buttonText}
              </button>
                ...
                {produto.features.map((feature) => (
                  <div key={feature.name} className="relative pl-9">
                    <dt className="inline font-semibold text-gray-900">
                      <FontAwesomeIcon
                        {...feature.iconProps}
                        className="absolute left-1 top-1 h-5 w-5 text-grayUso"
                        aria-hidden="true"
                      />
                      {feature.name}
                    </dt>{" "}
